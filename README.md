...
- **生体名なしのサイズ・価格質問への対応**:
  - 生体名がなく直接サイズ・価格を聞かれた場合（例: 「横45、奥行き30、高さ20のケージの値段」等）は、標準仕様（TypeA等）をもとに概算価格を回答した上で、「生体の種類によって適切な広さや必要な仕様（熱源・鍵・通気性など）が異なるため、飼育予定の生体を教えてください」とヒアリングを促して生体に応じた対話フローへ誘導します（「ケージ専用窓口です」等と断らない）。
