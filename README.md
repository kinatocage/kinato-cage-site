...
- **3Dケージ設計・見積もりシミュレーター (`/sim`)**: Three.jsを用いたリアルタイム3Dビューア。寸法・パネル素材・補強・キャスター・止まり木・モレ対策ゴムパッキン・２室分け（後付け仕切り板）等の豊富なオプションや床面防水コーキング標準装備を視覚的にプレビューし、概算価格と重量を瞬時に算出。
...
│   └── venv/             # Python 仮想環境
├── cage_mitumori/        # 3Dケージ設計・見積もりシミュレーター (Three.js / Vite)
│   ├── cost_materials_config.js # 資材原価・工賃マスタ
│   ├── src/              # シミュレーターのソースコード (CageModel, UI等)
│   └── index.html        # シミュレーターUI構造
├── public/               # 静的アセット (画像、ファビコン等)
│   └── sim/              # シミュレータービルド成果物（自動同期）
