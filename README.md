...
- **3Dケージ設計・見積もりシミュレーター (`/sim`)**: Three.jsを用いたリアルタイム3Dビューア。寸法・パネル素材・側面補強（側面上部開口高さ指定・範囲拡大）・キャスター・止まり木・モレ対策ゴムパッキン・２室分け（後付け仕切り板）等の豊富なオプションや床面防水コーキング標準装備を視覚的にプレビューし、概算価格と重量を瞬時に算出。スマホ向け縦型見積もりカードのPNG画像保存（iPhone「写真」アプリ保存・Android対応、Web Share API & 長押しプレビュー）および仕様テキストワンタップコピー機能も搭載。
...
- **アルミ複合板の不使用・採用不可（生体への配慮方針）**:
  - 「側面や背面をアルミ複合板にできますか？」「木製に近いブラウンや木目調のアルミ複合板にできますか？」といった質問に対しては、当店では**アルミ複合板は一切採用・使用していない**ことを明確に回答します。
  - お客様への説明方針：アルミ複合板は遮熱性が高く室内が温まりやすい性質がある反面、表面の熱伝導率が高すぎるため、室温が高い場合は生体が触れた際に熱く感じてしまい、逆に室温が低い場合は体温を奪いやすいという懸念があります。当店では生体にとって触れた際にも程よい暖かさの手触りとなるよう、アクリル板（または中空ポリカ）を採用しています。
- **ガラス素材の取り扱い**:
