...
- **3Dケージ設計・見積もりシミュレーター (`/sim`)**: Three.jsを用いたリアルタイム3Dビューア。寸法・パネル素材・補強・キャスター・止まり木・モレ対策ゴムパッキン・２室分け（後付け仕切り板）等の豊富なオプションや床面防水コーキング標準装備を視覚的にプレビューし、概算価格と重量を瞬時に算出。スマホ向け縦型見積もりカードのPNG画像保存（iPhone「写真」アプリ保存・Android対応、Web Share API & 長押しプレビュー）および仕様テキストワンタップコピー機能も搭載。
